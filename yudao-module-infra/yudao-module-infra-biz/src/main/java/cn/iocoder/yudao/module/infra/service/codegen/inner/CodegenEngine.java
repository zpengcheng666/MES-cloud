...
public class CodegenEngine {
    ...
    void initGlobalBindingMap() {
        // 全局配置
        globalBindingMap.put("preModuleName", codegenProperties.getPreModuleName());
    ...
    private String formatFilePath(String filePath, Map<String, Object> bindingMap) {
        ...
        filePath = StrUtil.replace(filePath, "${preModuleName}",
                getStr(bindingMap, "preModuleName"));
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "${preModuleName}-${table.moduleName}/" + // 顶级模块
                "${preModuleName}-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "${preModuleName}-${table.moduleName}/" + // 顶级模块
                "${preModuleName}-${table.moduleName}-biz/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "${preModuleName}-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "${preModuleName}-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
