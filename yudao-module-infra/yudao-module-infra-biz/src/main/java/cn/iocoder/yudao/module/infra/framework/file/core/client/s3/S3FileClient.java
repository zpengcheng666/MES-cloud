...
public class S3FileClient extends AbstractFileClient<S3FileClientConfig> {
    ...
    @Override
    public String upload(byte[] content, String path, String type,String bucket) throws Exception {
        String finalBucket = StringUtils.isEmpty(bucket)?config.getBucket():bucket;

        // 执行上传
        client.putObject(PutObjectArgs.builder()
                .bucket(finalBucket) // bucket 必须传递
                .contentType(type)
                .object(path) // 相对路径作为 key
                .stream(new ByteArrayInputStream(content), content.length, -1) // 文件内容
                .build());
        // 拼接返回路径
        return config.getEndpoint()+"/" +finalBucket+ "/" + path;
    }
