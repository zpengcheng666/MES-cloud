...
public interface AdminUserApi extends AutoTransable<AdminUserRespDTO> {
    ...
    CommonResult<Long> createUser(@RequestBody UserSaveReqVOTemp reqVO);

    @GetMapping(PREFIX + "/getByUsername")
    @Operation(summary = "通过 用户账号 查询用户")
    @Parameter(name = "username", description = "用户账号", example = "1", required = true)
    CommonResult<AdminUserRespDTO> getByUsername(@RequestParam("username") String username);
